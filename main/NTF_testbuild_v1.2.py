...
def read_data_from_folder(folder_path):
    ...
    for file in data_files:
        ...
        try:
            data = pd.read_csv(file_path, sep=r'\s+', names=['Frame', 'Time', 'X', 'Y'])  # Use raw string for regex
...
class NematodeTrackerApp:
    ...
    def show_clustering_info(self, event):
        ...
        if method in ['KMeans', 'Hierarchical', 'Gaussian Mixture', 'Spectral', 'BIRCH']:
        ...
        try:
            ...
            if aggregated_data:
                ...
                if clustering_method == 'DBSCAN':
                    labels = perform_clustering(normalized_aggregated_df, clustering_method, eps=eps, min_samples=min_samples)
                else:
                    labels = perform_clustering(normalized_aggregated_df, clustering_method, n_clusters=n_clusters)
